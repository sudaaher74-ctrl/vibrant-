...
if(canvas){
 ...
 renderer.outputColorSpace=THREE.SRGBColorSpace;toneMap=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.05;
 const root=new THREE.Group();scene.add(root);
 const warm=new THREE.Color(0xd5b477), dark=new THREE.Color(0x161513), marble=new THREE.Color(0xb8b1a3), glass=new THREE.Color(0x668078);
 ...
 const back=new THREE.Mesh(new THREE.BoxGeometry(24,7,0.3),mat(0x211f1b,.65));back.position.set(0,3.5,-5);back.receiveShadow=true;root.add(back);
 ...
 // jewellery display counters
 for(let i=-2;i<=2;i+=2){const base=new THREE.Mesh(new THREE.BoxGeometry(2.8,1.05,1.2),mat(0x2a2824,.32));base.position.set(i,0.53,-.8);base.castShadow=true;root.add(base);const top=new THREE.Mesh(new THREE.BoxGeometry(2.9,.07,1.3),mat(0xb5aa99,.2,.15));top.position.set(i,1.08,-.8);top.castShadow=true;root.add(top);const glassBox=new THREE.Mesh(new THREE.BoxGeometry(2.45,.65,.95),new THREE.MeshPhysicalMaterial({color:0x7d968c,transparent:true,opacity:.2,roughness:.08,metalness:.05,transmission:.35,thickness:.2}));glassBox.position.set(i,1.43,-.8);root.add(glassBox)}
 // reception
 ...
 // ceiling lights
 for(let x=-5;x<=5;x+=2.5){const light=new THREE.RectAreaLight(0xffe7bc,5,1.4,.18);light.position.set(x,6.7,-.2);light.rotation.x=Math.PI;root.add(light)}
 ...
 const fill=new THREE.HemisphereLight(0xfff2d8,0x1b201f,1.2);root.add(fill);
 // plants / vertical sculptural objects
 ...
 let targetX=7,targetY=3.1,targetZ=11,scrollProgress=0;
 function updateScene(){const max=Math.max(document.documentElement.scrollHeight-innerHeight,1);scrollProgress=Math.min(Math.max(scrollY/max,0),1);const local=Math.min(scrollProgress*2.7,1);targetX=7-5.2*local;targetY=3.1+.6*Math.sin(local*Math.PI);targetZ=11-5.2*local;root.rotation.y=-.18*local;}
 addEventListener('scroll',updateScene,{passive:true});
 addEventListener('mousemove',e=>{const nx=e.clientX/innerWidth-.5,ny=e.clientY/innerHeight-.5;targetX+=nx*.35;targetY-=ny*.15},{passive:true});
